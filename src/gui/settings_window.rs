...
impl SettingsWindow {
    ...
    pub fn title(&self) -> String {
        t!("settings_title", name = PROGRAM_NAME).to_string()
    }
    ...
    fn view(&self) -> Element<Message> {
        match self.screen {
            SettingsScreen::Home => self.settings_screen(),
            SettingsScreen::Location => self.location_screen(),
        }
    }

    fn settings_screen(&self) -> Element<Message> {
        column![text("title").size(50)].spacing(20).into()
    }

    fn location_screen(&self) -> Element<Message> {
        column![text("title").size(50)].spacing(20).into()
    }
}
